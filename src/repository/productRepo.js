...
export const GetById = async (db, id) => {
	const { data, error, status } = await createSupabaseClient(db).from('product').select().eq('id', id);
	if (error) throw error;
	return { data, status };
}
...
export const GetByCategory = async (db, category, from=0, to=20, order=true) => {
	const { data, error, status } = order == null
	? await createSupabaseClient(db).from('product').select('*').eq('category', category).range(from, to)
	: await createSupabaseClient(db).from('product').select('*').eq('category', category).order('price', { ascending: order }).range(from, to);
	if (error) throw error;
	return { data, status };
}
